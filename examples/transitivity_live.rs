...
//! small entity set → choice probabilities → the WST/MST/SST receipts.
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let model = std::env::args().nth(1).ok_or("usage: transitivity_live <model> [k]")?;
    let k: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(10);
    ...
    for a in 0..items.len() {
        for b in (a + 1)..items.len() {
            ...
            eprintln!("pair ({a},{b}): n={} p̂={:.2}", draws.len(),
    ...
    println!(
        ...
        report.wst_violations, report.wst_violations_2se,
        report.mst_violations, report.mst_violations_2se,
        report.sst_violations, report.sst_violations_2se,
        report.min_draws,
        cost as f64 / 1e9,
    );
