...
llmsorting means exactly the rows below; the drive is scored against
...
| Row | Status |
|---|---|
| Every published number recomputable from committed evidence (JSONL + replayable cache in dated `research/artifacts/live/` packs) | **covered** |
...
| Counts and catalogs in prose derive from source (verb lists, test counts, template lists) or carry a sync test | **named-gap** — defect sweep 2026-07-09 enumerates drift; sync pins to follow |
| Claims killed by the 2026-07-09 red team corrected at every downstream surface | **named-gap** until the sweep returns clean |
...
| The one verb (`believe`/`audit`: probe battery behind one word, verdict-first summary) | **named-gap**, gated on held-out validation — bars pre-registered in `research/notes/council-essence-2026-07-09/SYNTHESIS.md`; time-axis timer runs in background, gates nothing else |
...
| Portable evidence uses a three-level contract: `JudgmentPacket` is the content-addressed evidence atom; a thin `JudgmentStudyRecord` Merkle root commits independently addressable manifests, observations, packets, projections, and derivations; append-only ledger events are consumer orchestration, not the public evidence format | **partially covered** — the finite-candidate, single-axis `cardinal.judgement-run.v1` terminal atom now persists normalized inputs, exact execution evidence, accounting, and replayable output; the content-addressed study record, trace→packet adapter, offline recomposition, and generic renderer remain a named gap tracked in #52 |
...
| Row | Status |
|---|---|
| Honest absences load-bearing: the ✗ rows stay; noise floors quoted with every axis | **covered** |
...
Step 1 (read everything): running as a fresh-eyes sweep, findings to
`research/notes/canonicalize-2026-07-09/defects.md`. Step 2: this file. Step 3
(truth enforcement) and step 5 (voice scrape) follow the sweep. Step 6
(adversarial rounds until a round finds nothing) not yet run — this drive
is PARTIAL until that gate is measured, and says so here.
