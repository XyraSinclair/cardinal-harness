...
This example is deliberately small. The point is not to demonstrate a heroic benchmark; it is to show the moving parts of a `llmsort` rerank run without pretending that a synthetic transcript is a real model result.
...
```bash
cargo run --bin llmsort -- cache-export --out worked-example-cache.jsonl
```
