...
use llmsort::gateway::{Attribution, ChatGateway, ChatModel, ChatRequest, Message};
...
pub enum SlateError {
    ...
    Provider(#[from] llmsort::gateway::error::ProviderError),
...
async fn chat_json(
    ...
    for _attempt in 0..2 {
        ...
        if let Some(value) = llmsort::rerank::proposal_json::lenient_value(&response.content) {
...
fn string_array(value: &serde_json::Value) -> Result<Vec<String>, SlateError> {
    llmsort::rerank::proposal_json::value_string_array(value)
        .filter(|s| !s.is_empty())
