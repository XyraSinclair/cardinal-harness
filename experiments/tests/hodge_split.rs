...
use llmsort::rating_engine::{compute_hodge_split, Config};
...
fn solver_summary_split_sums_to_hcr_on_a_planted_cyclic_judge() {
    ...
    use llmsort::rating_engine::{AttributeParams, Observation, RaterParams, RatingEngine};
...
fn atlas_winner_c8_134_hosts_both_diagnostics_at_the_same_budget() {
    ...
    let spectral = llmsort::rating_engine::spectral_diagnostics(&edges, &ones, n, 256).unwrap();
...
fn jcb_graph_design_cannot_host_harmonic_disagreement() {
    ...
    let pairs = llmsort_experiments::core_pairs();
    let m = pairs.len();
