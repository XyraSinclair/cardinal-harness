...
fn judge_show_prompt_prints_seriate_evidence_bytes_before_missing_key_failure() {
    ...
    assert!(!output.status.success());
    let stderr = String::from_utf8(output.stderr).unwrap();
    let rendered = llmsort::seriate::instrument::Instrument::render(
        &llmsort::seriate::instrument::ratio_letter::RatioLetterInstrument,
        &llmsort::seriate::Attribute::new("judge", "shininess"),
        &llmsort::seriate::Entity::new("dull TIN spoon"),
        &llmsort::seriate::Entity::new("shiny GOLD ring"),
    );
...
async fn draws_measure_context_sensitivity_and_keep_the_prefix_stable() {
    ...
    assert!(
        sys0 >= llmsort::rerank::sampling::CACHE_FLOOR_CHARS,
